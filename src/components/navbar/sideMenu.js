...
const SideMenu = (props) => {
    ...
    // internal links
    const education = <Link to="/#education"><FaUserGraduate className={iconStyle}/></Link>
    const portfolio = <Link to="/#portfolio"><HiCode className={iconStyle}/></Link>
    const contact = <Link to="/#contact"><GrMail className={iconStyle}/></Link>

    return (
        <div className={sideMenu} style={props.open ? {'maxWidth': '2.5rem'} : {'maxWidth':0}}>
